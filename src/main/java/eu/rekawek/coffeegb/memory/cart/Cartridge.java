...
public class Cartridge implements AddressSpace {
    ...
    public Cartridge(GameboyOptions options) throws IOException {
        final File file = options.getRomFile();
        this.romFile = file;
    ...
    public static GameboyTypeFlag getRomType(File f) throws IOException {
        final int[] rom = loadFile(f);
        return GameboyTypeFlag.getFlag(rom[0x0143]);
    }
