        ...
        while (true) {
            ...
            switch (state) {
                case OPCODE:
                    ...
                    if (opcode1 == 0xcb) {
                        ...
                        if (currentOpcode == null) {
                            Minecraft.getMinecraft().player.sendMessage(new TextComponentString("[Gameboy Emulator] "+String.format("No command for 0x%02x", opcode1)).setStyle(new Style().setColor(TextFormatting.RED)));
                        }
                    }
                    if (!haltBugMode) {
                ...
                case EXT_OPCODE:
                    ...
                    if (currentOpcode == null) {
                        Minecraft.getMinecraft().player.sendMessage(new TextComponentString("[Gameboy Emulator] "+String.format("No command for %0xcb 0x%02x", opcode2)).setStyle(new Style().setColor(TextFormatting.RED)));
                    }
                    state = State.OPERAND;
                    registers.incrementPC();
                    break;
